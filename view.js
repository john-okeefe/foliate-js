...
export const makeBook = async (file) => {
  ...
  if (file.isDirectory) {
    ...
    if (isCBZ(file)) {
      // Check if CBZ is EPUB-structured (has content.opf)
      const hasOPF = loader.entries.some((e) => e.filename === "content.opf");

      if (hasOPF) {
        console.log("[view] CBZ has OPF, treating as EPUB");
        const { EPUB } = await import("./epub.js");
        book = await new EPUB(loader).init();
      } else {
        console.log("[view] CBZ is image-only, using comic-book loader");
